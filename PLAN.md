...
- **Form1098T, Form1098E and HomeOfficeDeduction have no tax year.** The estimate applies them to whichever year is requested, so a 2025 summary shows the same scholarship as 2026. Fixing it means adding `taxYear`, replacing `userId @unique` with `@@unique([userId, taxYear])`, and backfilling existing rows — which turns those relations into arrays and ripples into the summary route, the tax adapter, and the form upsert routes. **Deferred to after Phase 3 merges, owned by Opus 5**, because it edits the same API routes Gemini holds during Phase 3. (Phase 3.5)
