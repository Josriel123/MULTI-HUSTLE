...
Done 2026-09-24 (D53): sign-in checks moved out of the proxy, which Clerk 7 had deprecated (`createRouteMatcher`): the `(app)` layout protects the app's pages on the server and in the browser, every API route checks for itself, and a test keeps it that way.
