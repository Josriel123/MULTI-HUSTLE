 ...
 *   incomeBySource        income counted, per hustle
...
export function summaryPayload(rows: EstimateInputRows, routeWarnings: readonly Warning[] = []) {
  ...
  return {
    ...
    incomeBySource: built.incomeByHustle.map((h) => ({ name: h.name, type: h.type, income: toNumber(h.income), count: h.count })),
    transactions: {
...
/** A W-2 amount for the first `months` months, spread evenly: a W-2 is annual and carries no dates. */
function spread(value: MoneyInput | null | undefined, months: number): MoneyInput | null | undefined {
  return value === null || value === undefined ? value : money(value).times(months).dividedBy(12).toDecimalPlaces(2);
}

function w2Through(rows: readonly W2FormRow[], months: number): W2FormRow[] {
  return rows.map((row) => ({
    ...row,
    wages: spread(row.wages, months)!,
    federalWithheld: spread(row.federalWithheld, months),
    socialSecurityWages: spread(row.socialSecurityWages, months)!,
    socialSecurityTips: spread(row.socialSecurityTips, months),
    medicareWages: spread(row.medicareWages, months)!,
    medicareWithheld: spread(row.medicareWithheld, months),
  }));
}
 ...
 * by the date paid, and W-2 amounts spread evenly (`w2Spread` tells the page to
 * say so). December takes everything, payments made the following January
 * included, so its point is by construction the summary's figure.
 */
export function chartPayload(rows: EstimateInputRows) {
  const w2Forms = rows.w2Forms ?? [];
  const payments = rows.estimatedPayments ?? [];
  const points: ChartPoint[] = MONTH_NAMES.map((month, monthIndex) => {
    const december = monthIndex === 11;
    const monthEndExclusive = new Date(Date.UTC(rows.taxYear, monthIndex + 1, 1));
    const { estimate, safeToSpend } = estimateFromRows({
      ...rows,
      transactions: rows.transactions.filter((tx) => tx.date < monthEndExclusive),
      mileageLogs: (rows.mileageLogs ?? []).filter((log) => log.date < monthEndExclusive),
      w2Forms: december ? w2Forms : w2Through(w2Forms, monthIndex + 1),
      estimatedPayments: december ? payments : payments.filter((p) => p.paidOn < monthEndExclusive),
    });
