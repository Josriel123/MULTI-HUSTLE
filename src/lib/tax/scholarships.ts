import { isAboveZero, nonNegativeMoney, notBelowZero, ZERO, type Money, type MoneyInput } from './money';
 ...
 * does not report.
 *
 *   taxable = max(0, Box 5 - Box 1 - required course materials)
 *
 ...
 *    less tuition than Box 1 shows; the student is allowed to allocate the
 *    scholarship to tuition first (Pub. 970 ch. 1, Worksheet 1-1), so this
 *    is the taxpayer-favourable but permitted reading.
...
export function computeTaxableScholarships(input: ScholarshipInput): ScholarshipResult {
  ...
  const taxable = notBelowZero(box5.minus(qualified));
  ...
  return {
    ...
    lines,
    assumptions: [
      'Form 1098-T Box 1 is treated as the qualified tuition the scholarship paid; the scholarship is allocated to tuition before living costs (Pub. 970 ch. 1).',
    ...
    warnings,
    citations: [SCHOLARSHIP_CITATIONS.statute, SCHOLARSHIP_CITATIONS.services, SCHOLARSHIP_CITATIONS.pub970, SCHOLARSHIP_CITATIONS.form1098T],
  };
}
