...
function addMode(add: string | null): TransactionDialogMode | null {
  return add === 'income' ? { kind: 'create', type: 'Income' } : add === 'expense' ? { kind: 'create', type: 'Expense' } : null;
}
...
export default function TransactionsPage() {
  ...
  const [dialog, setDialog] = useState<TransactionDialogMode | null>(() => addMode(add));
  // The phone's "+" menu links here with ?add= while this page may already be
  // open, which does not remount it: follow the parameter, not just the first
  // render. Closing the dialog drops it from the URL, so the next tap is a change.
  const [seenAdd, setSeenAdd] = useState(add);
  if (add !== seenAdd) {
    setSeenAdd(add);
    const mode = addMode(add);
    if (mode) setDialog(mode);
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <TransactionDialog mode={dialog} onClose={closeDialog} hustles={hustles.data ?? []} taxYear={taxYear} onSaved={refreshAll} />
